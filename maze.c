...
int testfunction(Map *maze)
{
 ...
 for(int row=1;row<=maze->rows;row++)
 {
  ...
  {   
   if((col==1 && !isborder(maze,row,col,LEFTODD)) || (col==maze->cols && !isborder(maze,row,col,RIGHTODD)) || (row==1 && (row+col)%2==0 && !isborder(maze,row,col,TOP)) || (row==maze->rows && (row+col)%2==1 && !isborder(maze,row,col,BOTTOM)))hasentrance=true; 
...
int startborder(Map *maze,int row,int col,int leftright)
{
 ...
 if(maze->cells[0]==0)maze->cells[0]=4;//horni levy uhel
 if((maze->cols%2)==1 && maze->cells[maze->cols-1]==0)maze->cells[maze->cols-1]=4;//horni pravy uhel
 if((maze->rows)%2==0 && maze->cells[(maze->rows-1)*maze->cols]==0)maze->cells[(maze->rows-1)*maze->cols]=4;//dolni levy uhel
 if((maze->rows+maze->cols)%2==1 && maze->cells[maze->rows*maze->cols-1]==0)maze->cells[maze->rows*maze->cols-1]=4;//dilni pravy uhel
 ...
 {
   if((row==1 && (row+col)%2==1) || (row==maze->rows && (row+col)==0))return ERROR;
   if(row==1 && col%2 && !isborder(maze,row,col,'u'))return (lr)? LEFTEVEN:RIGHTEVEN;else
